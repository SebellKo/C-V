...
- popup UI는 React와 shadcn/ui 사용
- popup viewport는 `300 × 380px`로 고정
- `chrome.storage.local` 기반 로컬 영속 저장
- popup, content script, background service worker 실행 환경 필요
- 외부 서버와 원격 동기화 없음

DnD library, 상태 관리, module 책임, 메시지 계약, 저장 schema, 동시성 처리, 테스트 전략과 build 구성은 실제 구현을 진행하면서 이 문서에 함께 설계합니다.
